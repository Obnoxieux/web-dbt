<script lang="ts">
    ...
    export let data
    const createdDate = new Date(data.repo.created_at).toLocaleString()
    const updatedDate = new Date(data.repo.updated_at).toLocaleString()
</script>

<section class="mb-4 mt-2 flex flex-col gap-2">
    <div class="indicator">
        {#if data.repo.archived === true}
            <span class="indicator-item badge badge-neutral">Archive</span>
        {/if}
        <h1 class="h1 pt-3">{data.repo.name}</h1>
    </div>
    <div class="mt-4 max-w-3xl space-y-6">
        {data.repo.description ?? "The repository does not have a description."}
    </div>
</section>
<div class="divider"></div>
<h2 class="subheader col-span-4 mb-6 text-typescript">General Info</h2>
<section id="project-stats" class="grid grid-cols-4 gap-6">
    <div>Language</div>
    <div class="col-span-3">{data.repo.language}</div>
    <div>Tags</div>
    <div class="col-span-3 gap-2 flex flex-wrap">
        {#if data.repo.topics.length > 0}
            {#each data.repo.topics as topic}
    ...
    <div>Archived</div>
    <div class="col-span-3">{data.repo.archived ? "Yes" : "No"}</div>
    <div class="divider col-span-4 my-1"></div>
    <h2 class="subheader col-span-4 text-typescript">Links</h2>
    <div class="col-span-4 flex gap-2 md:gap-3">
        {#if data.repo.homepage !== ""}
            <a href="{data.repo.homepage}" target="_blank" role="button" class="btn btn-neutral">Homepage</a>
        {/if}
        <a href="{data.repo.html_url}" target="_blank" role="button" class="btn btn-neutral">Repository</a>
    </div>
</section>